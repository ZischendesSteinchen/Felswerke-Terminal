...
| Finnhub | `finnhub.io/api/v1/*` | `stocks/route.ts`, `stock-history/route.ts` | Aktien-Quotes, Candle-Daten |
...
| Yahoo Finance | `query1.finance.yahoo.com/*` | `stocks/route.ts`, `stock-history/route.ts` | Fallback fuer Aktien-Quotes und Candles |
...
| `/api/stocks` | `stocks/route.ts` | Finnhub Aktien-Daten, Yahoo Finance Fallback (cached, TTL 300s) |
| `/api/stock-history` | `stock-history/route.ts` | Aktien-Chartdaten: Finnhub Candles, Yahoo Finance Fallback (cached, 300s) |
...
### Aktien-Quotes (`stocks/route.ts`)
```
Finnhub → Yahoo Finance (Fallback)
```
> Finnhub `demo`-Key liefert oft `c=0` → Yahoo Finance wird automatisch als Fallback genutzt.

### Aktien-Charts (`stock-history/route.ts`)
```
Finnhub Candles → Yahoo Finance Chart (Fallback)
```
> Gleiche Logik wie bei Quotes — Yahoo greift ein wenn Finnhub keine Daten liefert.
