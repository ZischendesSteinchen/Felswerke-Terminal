...
// --- Error Boundary ---
interface EBProps { children: ReactNode; widgetTitle: string }
interface EBState { hasError: boolean; error?: Error }

class WidgetErrorBoundary extends Component<EBProps, EBState> {
  state: EBState = { hasError: false };
  static getDerivedStateFromError(error: Error): EBState { return { hasError: true, error }; }
  componentDidCatch(error: Error, info: ErrorInfo) { console.error(`[Widget "${this.props.widgetTitle}"] crashed:`, error, info); }
  render() {
    if (this.state.hasError) {
      return (
        <div className="h-full flex flex-col items-center justify-center gap-2 p-4 text-center">
          <span className="text-terminal-red text-xs font-medium">Widget crashed</span>
          <span className="text-[10px] text-terminal-muted max-w-[200px] truncate">{this.state.error?.message}</span>
          <button
