        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              className="w-[calc(100%-2rem)] max-w-[560px] max-h-[70vh] bg-terminal-panel border border-terminal-border rounded-lg shadow-2xl flex flex-col overflow-hidden"
            >
              {/* Header */}
              ...
              {/* Widget List */}
              <div className="flex-1 overflow-auto p-4 grid grid-cols-2 auto-rows-fr gap-3 content-start">
                {filteredDefs.map((def) => (
                  <button
                    key={def.type}
                    onClick={() => handleAdd(def)}
                    className="flex items-start gap-3 p-4 bg-terminal-bg border border-terminal-border rounded hover:border-terminal-accent/40 hover:bg-terminal-accent/5 transition-all text-left group"
                  >
