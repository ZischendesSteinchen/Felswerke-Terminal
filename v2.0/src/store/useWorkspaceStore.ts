...
interface WorkspaceState {
  ...
  setActiveSettingsWidget: (id: string | null) => void;

  // Workspace actions
  addWorkspace: (name: string) => string;
  addWorkspaceFromTemplate: (template: WorkspaceTemplate) => string;
...
export const useWorkspaceStore = create<WorkspaceState>((set, get) => ({
  ...
  setActiveSettingsWidget: (id) => set({ activeSettingsWidgetId: id }),
  ...
  addWorkspaceFromTemplate: (template) => {
    const wsId = `ws-${uuid().slice(0, 8)}`;
    const now = Date.now();
    const newWidgets: Record<string, Widget> = {};
    const widgetIds: string[] = [];

    for (const w of template.widgets) {
      const wId = `w-${uuid().slice(0, 8)}`;
      newWidgets[wId] = {
        id: wId,
        type: w.type,
        title: w.title,
