...
export async function fetchUsdToEurRate(): Promise<number> {
  try {
    const res = await fetch('https://api.coinbase.com/v2/exchange-rates?currency=USD', { ...NO_STORE_FETCH, signal: withTimeout(5000) });
...
export async function fetchBinancePrices(usdToEurRate: number): Promise<ExchangePrice[]> {
  try {
    const [btcRes, ethRes] = await Promise.all([
      fetch('https://api.binance.com/api/v3/ticker/price?symbol=BTCUSDT', { ...NO_STORE_FETCH, signal: withTimeout(5000) }),
      fetch('https://api.binance.com/api/v3/ticker/price?symbol=ETHUSDT', { ...NO_STORE_FETCH, signal: withTimeout(5000) }),
    ]);
...
export async function fetchCoinbasePrices(): Promise<ExchangePrice[]> {
  try {
    const [btcRes, ethRes] = await Promise.all([
      fetch('https://api.coinbase.com/v2/prices/BTC-EUR/spot', { ...NO_STORE_FETCH, signal: withTimeout(5000) }),
      fetch('https://api.coinbase.com/v2/prices/ETH-EUR/spot', { ...NO_STORE_FETCH, signal: withTimeout(5000) }),
    ]);
...
export async function fetchCoinGeckoPrices(): Promise<ExchangePrice[]> {
  try {
    const res = await fetch(
      ...
      { ...NO_STORE_FETCH, signal: withTimeout(5000) }
    );
...
export async function fetchBybitPrices(usdToEurRate: number): Promise<ExchangePrice[]> {
  try {
    const [btcRes, ethRes] = await Promise.all([
      fetch('https://api.bybit.com/v5/market/tickers?category=spot&symbol=BTCUSDT', { ...NO_STORE_FETCH, signal: withTimeout(5000) }),
      fetch('https://api.bybit.com/v5/market/tickers?category=spot&symbol=ETHUSDT', { ...NO_STORE_FETCH, signal: withTimeout(5000) }),
    ]);
...
export async function fetchGateioPrices(usdToEurRate: number): Promise<ExchangePrice[]> {
  try {
    const [btcRes, ethRes] = await Promise.all([
      fetch('https://api.gateio.ws/api/v4/spot/tickers?currency_pair=BTC_USDT', { ...NO_STORE_FETCH, signal: withTimeout(5000) }),
      fetch('https://api.gateio.ws/api/v4/spot/tickers?currency_pair=ETH_USDT', { ...NO_STORE_FETCH, signal: withTimeout(5000) }),
    ]);
