...
export default function WidgetSettingsPanel() {
  ...
  // Listen for widget-settings events from WidgetWrapper
  useEffect(() => {
    const handler = (e: Event) => {
      const detail = (e as CustomEvent).detail;
      if (detail?.widgetId) setWidgetId(detail.widgetId);
    };
    window.addEventListener('widget-settings', handler);
    return () => window.removeEventListener('widget-settings', handler);
  }, []);

  const close = useCallback(() => setWidgetId(null), []);
